...
class TrainRecord {
  ...
  /// LBJ BCD speed uses '-' (nibble 0xD) / space as blank digits, not a minus sign.
  /// e.g. "-59" → "59", " 70" → "70", "NUL" → "".
  static String normalizeLbjSpeed(String? raw) {
    if (raw == null) return '';
    final s = raw.trim();
    if (s.isEmpty || s == 'NUL' || s == '<NUL>') return '';
  ...
  /// Prefer BLE/firmware loco_type; fall back to assets/loco_type_info.csv.
  String get resolvedLocoType {
    final existing = locoType.trim();
    if (existing.isNotEmpty && existing != '<NUL>') {
      return existing;
    }
    return LocoTypeUtil().getLocoTypeByLocoNumber(loco) ?? '';
  }

  String get locoTypeText {
    final resolved = resolvedLocoType;
    if (resolved.isEmpty) return '未知';
    return resolved;
  }
  ...
  String? get locoInfo {
    return LocoInfoUtil.getLocoInfoDisplay(resolvedLocoType, loco);
  }
